...
export default function QuestionBankIndex({ questionBanks }: Props) {
  ...
  const filteredBanks = questionBanks.data.filter(bank =>
      ...
      <div className="flex h-full flex-1 flex-col gap-6 p-4">
        ...
        <div className="flex flex-col gap-2">
          ...
          <p className="text-sm text-muted-foreground">
            Manage your question banks and organize your questions effectively.
          </p>
        </div>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {filteredBanks.map((bank) => (
              <Card key={bank.id} className="flex flex-col justify-between">
                <CardHeader>
                  <CardTitle className="line-clamp-2">{bank.name}</CardTitle>
                  {bank.description && (
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex items-center justify-between text-sm text-muted-foreground">
                    <span>{bank.questions_count} question{bank.questions_count !== 1 ? 's' : ''}</span>
                  ...
                  <div className="flex gap-2">
                    <Button
                      size="sm"
                      variant="secondary"
                      onClick={() => router.get(`/teacher/question-banks/${bank.id}`)}
                      className="flex-1"
                    >
                      Details
                    </Button>
          ...
          <div className="flex items-center justify-center gap-2">
            {questionBanks.links.map((link, index) => (
              <Button
                ...
                onClick={() => {
                  if (link.url) {
                    const page = new URLSearchParams(new URL(link.url).search).get('page')
